...
<div class="container text-start">
  ...
  <div class="row pt-4">
    <div class="col">
      <h2 class="fs-1 fw-semibold">상품 수정</h2>
    </div>
  </div>
  ...
  <div class="row align-items-center">
    ...
    <div class="col-lg-10">
    	<div class="form-floating">
	      <input class="form-control form-control-sm mb-2" type="text" placeholder="상품 제목을 입력해주세요" aria-label="상품 제목" id="GOODS_TITLE" name="GOODS_TITLE" value="${GOODS.GOODS_TITLE}" maxlength="30" style="width:450px; height:40px;" required>
  ...
  <div class="row align-items-center">
    ...
    <div class="col-lg-4">
      <select class="form-select form-select-sm mb-2" aria-label="카테고리" name="GOODS_CATEGORY" required>
		  ...
		  <option value="상의" <c:out value="${GOODS.GOODS_CATEGORY eq '상의' ? 'selected' :''}"/>>상의</option>
		  <option value="하의" <c:out value="${GOODS.GOODS_CATEGORY eq '하의' ? 'selected' :''}"/>>하의</option>
		  <option value="신발" <c:out value="${GOODS.GOODS_CATEGORY eq '신발' ? 'selected' :''}"/>>신발</option>
		  <option value="악세사리" <c:out value="${GOODS.GOODS_CATEGORY eq '악세사리' ? 'selected' :''}"/>>악세사리</option>
	  </select>
	  <div class="invalid-feedback text-start">
		  카테고리를 선택해주세요
	  </div>
  ...
  <div class="row align-items-center">
    ...
    <div class="col-lg-4">
      <select class="form-select form-select-sm mb-2" aria-label="상태" name="GOODS_STATUS" required>
		  ...
		  <option value="A" <c:out value="${GOODS.GOODS_STATUS eq 'A' ? 'selected' :''}"/>>A</option>
		  <option value="B" <c:out value="${GOODS.GOODS_STATUS eq 'B' ? 'selected' :''}"/>>B</option>
		  <option value="C" <c:out value="${GOODS.GOODS_STATUS eq 'C' ? 'selected' :''}"/>>C</option>
	  </select>
	  <div class="invalid-feedback text-start">
		  상품 상태를 선택해주세요
	  </div>
    ...
    <div class="col-lg-4">
      <div class="input-group mb-2">
          <span class="input-group-text" style="width:40px; height:30px;"><i class="fa-solid fa-won-sign"></i></span>
          	<input type="text" class="form-control" maxlength="20" aria-label="Amount" id="GOODS_PRICE" name="GOODS_PRICE" value="${GOODS.GOODS_PRICE }" onkeyup="inputNumberFormat(this);" style="width:50px; height:30px;" required>
  ...
  <div class="row align-items-center">
    ...
    <div class="col-lg-10">
      <div class="form-floating mb-2">
		  <textarea class="form-control" placeholder="상품 설명을 적어주세요" id="GOODS_CONTENT" name="GOODS_CONTENT" style="height: 200px" required>${GOODS.GOODS_CONTENT }</textarea>
  ...
  <div class="row pt-4 pb-4 align-items-center">
  	<div class="col text-lg-end text-md-end text-sm-end">
  		<button type="submit" class="btn btn-danger" name="goodsDelete">상품삭제</button>
  		<button type="button" class="btn btn-secondary ms-2" name="goodsModify">상품수정</button>
  	</div>
  </div>
...
//썸네일 미리보기.
	...
	function dataSubmit() {
	    ...
	    $.ajax({
	    	...
	    	if (fragment.code == "OK"){ //응답결과
	            ...
	            fn_goodsModify(fragment.GOODS_NUM);
	        } else {
		...
		$.ajax({
		    ...
		    success: function(args){
		    	for(var i=0; i<args.length; i++) {
		    		if(args[i].exist){
				      	var data = args[i].blob;
				      	var metadata = {type: 'image/'+args[i].fileExtension};
				      	var filename = args[i].filename;
				      	var newFile = new File([data], filename+'_NS'+args[i].fileNum, metadata);
				      	fileInfoArr.push(newFile);
				      	console.log(fileInfoArr);
				        ...
				        $(".imgNum_"+i).attr("src", "data:image/png;base64," + data);
				        num = num + 1;
				    }else{
				      	alert("파일이 존재하지 않습니다.");
				    }
	...
	(() => {
		  ...
		  $("button[name='goodsModify']").on("click", function(e) { 
			e.preventDefault();
