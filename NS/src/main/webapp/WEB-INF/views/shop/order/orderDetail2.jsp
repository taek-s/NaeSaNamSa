  ...
  <body>
    <div class="container text-start p-5">
      ...
      >
        ...
        <input type="hidden" id="tid" name="tid" value='<c:out value="${tid}"/>'>
          <div class="col-10">
            <button type="button" class="btn btn-primary me-4" name="main">
             ...
             <c:if test="${order.ORDERS_USER == MEM_NUM}">
	            ...
	            <!-- 배송상태가 배송전일 경우에만 표시되도록 작성 -->
	         	<c:if test="${order.ORDERS_STATUS!='결제 완료'}">
	            	<button type="button" class="btn btn-warning me-4" name="orderModify">
	             	 주문수정
	           		</button>
	         	</c:if>
	         </c:if>
