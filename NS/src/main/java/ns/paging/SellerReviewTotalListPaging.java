...
@Component(value = "sellerReviewTotalListPaging")
public class SellerReviewTotalListPaging {
	// 공통 적용
	...
	public void makePagingHTML() {
		...
		if (startPage > pageBlock)
			pagingHTML.append("<li class='page-item'><a class='page-link' href='/ns/seller/info?pgr=" + (startPage - 1)
					+ "&goodsTstatus=" + goodsTstatus + "&MEM_NUM=" + sellerMemNum + "&pg=" + pg
					+ "'><span aria-hidden='true'>&laquo;</span></a></li>");

		for (int i = startPage; i <= endPage; i++) {
			if (i == currentPage) {
				pagingHTML.append("<li class='page-item active'><a class='page-link' href='" + "/ns/seller/info?pgr="
						+ i + "&goodsTstatus=" + goodsTstatus + "&MEM_NUM=" + sellerMemNum + "&pg=" + pg + "'>" + i
						+ "</a></li>");
			} else {
				pagingHTML.append(
						"<li class='page-item'><a class='page-link' href='/ns/seller/info?pgr=" + i + "&goodsTstatus="
								+ goodsTstatus + "&MEM_NUM=" + sellerMemNum + "&pg=" + pg + "'>" + i + "</a></li>");
			}

		}

		if (endPage < totalP) {
			pagingHTML.append("<li class='page-item'><a class='page-link' href='/ns/seller/info?pgr=" + (endPage + 1)
					+ "&goodsTstatus=" + goodsTstatus + "&MEM_NUM=" + sellerMemNum + "&pg=" + pg
